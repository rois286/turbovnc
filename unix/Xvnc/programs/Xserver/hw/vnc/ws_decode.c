 ...
 *  assumed, from Git logs and context, to have been authored by Andreas
 *  Weigel, Christian Beier, and Jocelyn Le Sage and made available under the
...
hybiReadHeader(ws_ctx_t *wsctx, int *sockRet, int *nPayload)
{
  ...
  if (ret <= 0) {
    if (-1 == ret) {
      /* save errno because rfbLog() will tamper it */
      int olderrno = errno;
  ...
  if (isControlFrame(wsctx)) {
    ...
    if (wsctx->header.fin == 0) {
       ...
       * fragmented. */
      rfbLog("control frame with FIN bit cleared received, aborting\n");
      errno = EPROTO;
      goto err_cleanup_state;
    }
  } else {
  ...
  **/
  if (!(wsctx->header.data->b1 & 0x80)) {
    rfbLog("%s: got frame without mask; ret=%d\n", __func__, ret);
    errno = EPROTO;
    goto err_cleanup_state;
  }
  ...
  if (wsctx->header.payloadLen < 126 && wsctx->header.nRead >= 6) {
    ...
    rfbLog("%s: incomplete frame header; ret=%d\n", __func__, ret);
    goto ret_header_pending;
  }
  ...
  if ((wsctx->header.headerLen > WS_HYBI_HEADER_LEN_SHORT
    ...
    rfbLog("%s: invalid length field; headerLen=%d payloadLen=%llu\n", __func__, wsctx->header.headerLen, wsctx->header.payloadLen);
    errno = EPROTO;
    goto err_cleanup_state;
  }
...
hybiReadAndDecode(ws_ctx_t *wsctx, char *dst, int len, int *sockRet, int nInBuf)
{
  ...
  if (nextRead > 0) {
    ...
    if (-1 == (n = wsctx->ctxInfo.readFunc(wsctx->ctxInfo.ctxPtr, wsctx->writePos, nextRead))) {
      int olderrno = errno;
      rfbLog("%s: read; %s", __func__, strerror(errno));
      errno = olderrno;
      *sockRet = -1;
      return WS_HYBI_STATE_ERR;
    } else if (n == 0) {
  ...
  if (toDecode < 0) {
    rfbLog("%s: internal error; negative number of bytes to decode: %d", __func__, toDecode);
    errno=EIO;
    *sockRet = -1;
    return WS_HYBI_STATE_ERR;
  }
  ...
  if (wsctx->hybiDecodeState == WS_HYBI_STATE_FRAME_COMPLETE) {
    ...
    if (wsctx->carrylen < 0 || wsctx->carrylen > ARRAYSIZE(wsctx->carryBuf)) {
      rfbLog("%s: internal error, invalid carry over size: carrylen=%d, toDecode=%d, i=%d", __func__, wsctx->carrylen, toDecode, i);
      *sockRet = -1;
      errno = EIO;
      return WS_HYBI_STATE_ERR;
    }
  ...
  switch (wsctx->header.opcode) {
    ...
    case WS_OPCODE_TEXT_FRAME:
      ...
      if (-1 == (wsctx->readlen = rfbBase64PtoN((char *)data, data, bufsize))) {
        rfbLog("%s: Base64 decode error; %s\n", __func__, strerror(errno));
      }
...
webSocketsDecodeHybi(ws_ctx_t *wsctx, char *dst, int len)
{
    ...
    switch (wsctx->hybiDecodeState){
      ...
      default:
        /* invalid state */
        rfbLog("%s: called with invalid state %d\n", __func__, wsctx->hybiDecodeState);
        result = -1;
        errno = EIO;
